...
export function DiffModal({
  ...
  const diffLines = (pendingApproval.diff_text || '').split('\n');
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400 mt-0.5">
                Subordinate <span className="text-white font-semibold">{subordinateAgent?.personName || pendingApproval.author_id}</span> ({subordinateAgent?.roleName}) submitting slice to <span className="text-white font-semibold">{supervisorAgent?.personName || 'Supervisor'}</span>
              </p>
            </div>
          </div>
