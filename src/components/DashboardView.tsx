...
import { 
  getRoleSchemaForAgent, 
  validateAgentTransitionFromIdle, 
  initializeSchemaStubs, 
  processAgentDocTags 
} from '../utils/roleSchemas';
...
export function DashboardView({ project, onBack, onUpdateProject, onDeleteProject, onEditTeam }: DashboardViewProps) {
  ...
  const [activeDocCategory, setActiveDocCategory] = useState<string>('arch');
  ...
  const handleUpdateActiveDocContent = (newContent: string) => {
    ...
    const archDoc = updatedDocs.find(d => d.id === 'arch' || d.category === 'architecture' || d.docType === 'architecture');
    const decisionsVal = archDoc ? archDoc.content : newContent;
  ...
  // Add a new document to active agent's memory bank
  const handleAddDocument = (newDoc: AgentDoc) => {
    if (!activeAgent) return;
    const currentDocs = getAgentDocs(activeAgent);
    const updatedDocs = [...currentDocs, newDoc];
    updateAgent(activeAgent.id, { documents: updatedDocs });
    setActiveDocCategory(newDoc.id);
  };

  // Soft-delete (archive) a document from active agent's memory bank with confirmation
  const handleDeleteDocument = (docId: string) => {
    ...
    if (!window.confirm(`Are you sure you want to soft-delete/archive "${targetDoc.title}"?`)) {
      return;
    }

    const updatedDocs = currentDocs.map(d => d.id === docId ? { ...d, isArchived: true, updatedAt: Date.now() } : d);
    updateAgent(activeAgent.id, { documents: updatedDocs });
    const fallbackDoc = updatedDocs.find(d => !d.isArchived);
    if (fallbackDoc) {
      setActiveDocCategory(fallbackDoc.id);
    }
  };

  // Chat message handler via API
  const handleSendMessage = async () => {
    if (!inputText.trim() || !activeAgent || isTyping) return;

    // Validate transition out of IDLE and auto-seed schema stubs if required
    let activeDocs = getAgentDocs(activeAgent);
    if (activeAgent.status === 'idle') {
      const validation = validateAgentTransitionFromIdle(activeAgent);
      if (!validation.canStart) {
        // Auto-seed missing schema required document stubs
        activeDocs = initializeSchemaStubs(activeAgent);
      }
    }
    ...
    const newHistory = [...activeAgent.chatHistory, userMessage];
    updateAgent(activeAgent.id, { 
      chatHistory: newHistory,
      documents: activeDocs,
      status: activeAgent.status === 'idle' ? 'drafting' : activeAgent.status
    });
    ...
    try {
      const parentAgent = activeAgent.parentId ? project.agents[activeAgent.parentId] : null;
      const rawReply = await api.sendChatMessage(
        activeAgent,
        newHistory,
        currentInput,
        project.id,
      ...
      const { updatedDocs, cleanedReply } = processAgentDocTags(rawReply, {
        ...activeAgent,
        documents: activeDocs
      });
  ...
  // BUILD ARCHITECTURE FOR ANY AGENT (Head Architect, Leads, Specialists, etc.)
  const handleBuildArchitectureForAgent = async (targetAgentId: string) => {
    ...
    try {
      const res = await api.createArchitecture(project.id, targetAgentId, targetAgentNode.responsibilities);
      
      const docs = getAgentDocs(targetAgentNode);
      let foundArch = false;
      let updatedDocs = docs.map(d => {
        if (d.id === 'arch' || d.category === 'architecture') {
          foundArch = true;
          return { ...d, content: res.masterBlueprint, updatedAt: Date.now() };
        }
        return d;
      });
      ...
      if (!foundArch) {
        updatedDocs.unshift({
          ...
          content: res.masterBlueprint,
          updatedAt: Date.now()
        });
      }

      const updatedAgentNode: AgentNode = {
        ...targetAgentNode,
        decisions: res.masterBlueprint,
        documents: updatedDocs,
        status: 'drafting',
        chatHistory: [
          ...targetAgentNode.chatHistory,
          {
            id: Date.now().toString(),
            role: 'agent',
            content: `🏗️ **[Domain Technical Architecture Generated]**:\n\n${res.masterBlueprint}`,
            timestamp: Date.now()
          }
        ]
      };
      ...
      const updatedAgents = {
        ...project.agents,
        [targetAgentId]: updatedAgentNode
      };

      onUpdateProject({
        ...project,
        agents: updatedAgents,
        ...(targetAgentId === project.rootAgentId ? { masterBlueprint: res.masterBlueprint } : {})
      });

      setActiveDocCategory('arch');
      setWorkbenchTab('preview');
    } catch (err: any) {
      setApiError(err.message || 'Failed to generate architecture.');
    } finally {
      setBuildingAgentsMap(prev => ({ ...prev, [targetAgentId]: false }));
    }
  };

  // Head Architect: Finalize & Delegate Downward
  const handleFinalizeArchitecture = async () => {
    ...
    try {
      const res = await api.finalizeArchitecture(project.id, activeAgent.id);
      
      const updatedAgents = { ...project.agents };
      updatedAgents[activeAgent.id] = {
        ...updatedAgents[activeAgent.id],
        status: 'delegated',
        chatHistory: [
          ...updatedAgents[activeAgent.id].chatHistory,
          {
            id: Date.now().toString(),
            role: 'agent',
            content: `✅ Master architecture finalized and sliced! Sub-plans delegated to direct reports: ${activeAgent.childrenIds.map(id => project.agents[id]?.personName).join(', ')}.`,
            timestamp: Date.now()
          }
        ]
      };

      // Apply generated sub-plans to direct reports
      for (const childId of activeAgent.childrenIds) {
        if (updatedAgents[childId]) {
          const subPlan = res.delegatedSlices[childId] || `# Delegated Sub-Plan: ${updatedAgents[childId].roleName}\n\nDerived from Master Blueprint.`;
          const childDocs = getAgentDocs(updatedAgents[childId]);
          const updatedChildDocs = childDocs.map(d => (d.id === 'arch' || d.category === 'architecture') ? { ...d, content: subPlan, updatedAt: Date.now() } : d);

          updatedAgents[childId] = {
            ...updatedAgents[childId],
            decisions: subPlan,
            documents: updatedChildDocs,
            status: 'drafting',
            chatHistory: [
              ...updatedAgents[childId].chatHistory,
              {
                id: Date.now().toString() + Math.random(),
                role: 'agent',
                content: `📋 **[Delegated Domain Sub-Plan]**:\n\n${subPlan}\n\n*Review and refine this spec in your workspace.*`,
                timestamp: Date.now()
              }
            ]
          };
      ...
      onUpdateProject({ ...project, agents: updatedAgents });
    } catch (err: any) {
      setApiError(err.message || 'Failed to finalize and delegate.');
    } finally {
  ...
  const handlePublishSlice = async () => {
    ...
    try {
      const res = await api.publishSlice(project.id, activeAgent.id, activeAgent.decisions);
      
      const updatedAgents = { ...project.agents };
      updatedAgents[activeAgent.id] = {
        ...updatedAgents[activeAgent.id],
        status: res.agentStatus as AgentStatus,
        ...
        chatHistory: [
          ...updatedAgents[activeAgent.id].chatHistory,
          {
            id: Date.now().toString(),
            role: 'agent',
            content: `📡 **[Domain Specification Published]**:\n\nSpecification broadcasted. Status set to '${res.agentStatus}'.`,
            timestamp: Date.now()
          }
        ]
      };

      // Record pending approval on supervisor if awaiting review
      const pendingApprovals = { ...(project.pendingApprovals || {}) };
      if (activeAgent.parentId && res.agentStatus === 'awaiting_review') {
        const supervisorId = activeAgent.parentId;
        if (!pendingApprovals[supervisorId]) pendingApprovals[supervisorId] = [];

        const existingIdx = pendingApprovals[supervisorId].findIndex(p => p.author_id === activeAgent.id);
        const approvalObj: PendingApproval = {
          slice_id: `slice-${activeAgent.id}`,
          supervisor_id: supervisorId,
          author_id: activeAgent.id,
          title: `Spec: ${activeAgent.roleName}`,
          content: activeAgent.decisions,
          diff_text: res.diffSummary || '--- Base Spec\n+++ Updated Spec',
          version: (activeAgent.version || 1) + 1,
          is_finalized: true
        };

        if (existingIdx >= 0) {
          pendingApprovals[supervisorId][existingIdx] = approvalObj;
        } else {
          pendingApprovals[supervisorId].push(approvalObj);
        }
      }
      ...
      setApiError(err.message || 'Failed to publish domain specification.');
    } finally {
      setIsPublishingSlice(false);
    }
  };

  // Supervisor: Approve & Merge Subordinate Slice
  const handleApproveSlice = async (subordinateId: string) => {
    ...
    try {
      const res = await api.approveSlice(project.id, activeAgent.id, subordinateId);

      const updatedAgents = { ...project.agents };
      if (updatedAgents[subordinateId]) {
        updatedAgents[subordinateId] = {
          ...updatedAgents[subordinateId],
          status: 'approved'
        };
      }

      const subordinate = project.agents[subordinateId];
      const mergedText = activeAgent.decisions + `\n\n### [APPROVED SLICE] ${subordinate?.roleName} (${subordinate?.personName})\n${subordinate?.decisions || ''}`;
      
      const supervisorDocs = getAgentDocs(activeAgent);
      const updatedSupervisorDocs = supervisorDocs.map(d => (d.id === 'arch' || d.category === 'architecture') ? { ...d, content: mergedText, updatedAt: Date.now() } : d);

      updatedAgents[activeAgent.id] = {
        ...updatedAgents[activeAgent.id],
        decisions: mergedText,
        documents: updatedSupervisorDocs,
        chatHistory: [
          ...updatedAgents[activeAgent.id].chatHistory,
          {
            id: Date.now().toString(),
            role: 'agent',
            content: `🎉 **[Approved & Merged Subordinate Slice]**: Merged specification from ${subordinate?.personName} (${subordinate?.roleName}) into supervisor architecture canvas.`,
            timestamp: Date.now()
          }
        ]
      };
      ...
      const pendingApprovals = { ...(project.pendingApprovals || {}) };
      if (pendingApprovals[activeAgent.id]) {
        pendingApprovals[activeAgent.id] = pendingApprovals[activeAgent.id].filter(
          p => p.author_id !== subordinateId
        );
      }
      ...
      setApiError(err.message || 'Failed to approve slice.');
    } finally {
    ...
    <div className="flex h-screen overflow-hidden bg-neutral-50 font-sans flex-col">
                  ...
                  {currentPendingApprovals.length > 0 && (
                    <div className="flex items-center gap-1.5 bg-purple-50 p-1 rounded-xl border border-purple-200/80">
                      ...
                      {currentPendingApprovals.map(approval => {
                        const subAgent = project.agents[approval.author_id];
                        return (
                          <button
                            key={approval.author_id}
                          ...
                          >
                            <Eye className="w-3 h-3" /> Diff: {subAgent?.personName || approval.author_id}
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="bg-neutral-100/70 border-b border-neutral-200 px-6 py-2 flex flex-wrap items-center justify-between gap-3 shrink-0">
                <div className="flex items-center gap-2 overflow-x-auto py-0.5 min-w-0">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-white hover:bg-indigo-50 hover:text-indigo-700 text-neutral-600 rounded-xl text-xs font-semibold transition-colors flex items-center gap-1 border border-dashed border-neutral-300 shrink-0"
                    title={`Add Custom Document to ${activeAgent.personName}'s Memory`}
                  >
                  ...
                  {activeDoc && (
                    <div className="flex items-center gap-1 bg-white p-0.5 rounded-xl border border-neutral-200/80 shadow-2xs">
                      ...
                      <button
                        ...
                        title={`Delete "${activeDoc.title}" from ${activeAgent.personName}'s memory`}
                      >
      ...
      {selectedPendingApproval && activeAgent && (
        <DiffModal
          ...
          subordinateAgent={project.agents[selectedPendingApproval.author_id]}
          supervisorAgent={activeAgent}
          onApprove={() => handleApproveSlice(selectedPendingApproval.author_id)}
      ...
      {/* Add Document Modal */}
      {showAddDocModal && activeAgent && (
        <AddDocumentModal
          agentName={activeAgent.personName}
