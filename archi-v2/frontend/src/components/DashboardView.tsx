...
export function DashboardView({ project, onBack, onUpdateProject, onDeleteProject, onEditTeam }: DashboardViewProps) {
  ...
  // Sync edit fields whenever the active agent's server state changes
  useEffect(() => {
    if (activeAgent) {
  ...
  // Banners belong to the agent that produced them, so they are cleared only
  // when the selection actually changes — not on every server refresh.
  useEffect(() => {
  ...
  const noteDegraded = (info?: { degraded: boolean; reason: string; provider: string }) => {
    ...
    const source = info.provider ? `'${info.provider}'` : 'a deterministic template';
    setDegradedNotice(
      `Degraded output — generated by ${source}, not a live model.${info.reason ? ` ${info.reason}` : ''}`
    );
  };
  ...
  // Supervisor: send a submission back down for rework
  const handleRequestRevision = async (subordinateId: string) => {
    if (!activeAgent) return;
    setIsRequestingRevision(true);
    setApiError(null);

    try {
      await api.requestRevision(project.id, activeAgent.id, subordinateId);
      await syncFromServer();
      setSelectedPendingApproval(null);
    } catch (err) {
      failWith(err, 'Failed to request a revision.');
    } finally {
    ...
    <div className="flex h-screen overflow-hidden bg-neutral-50 font-sans flex-col">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-neutral-50 relative overflow-hidden">
          {activeAgent ? (
            <>
              ...
              {/* Degraded output notice — the reply came from a template, not a model */}
              {degradedNotice && (
                <div className="bg-amber-50 border-b border-amber-200 px-6 py-2.5 flex items-center justify-between text-xs text-amber-800 shrink-0">
                  ...
                  <button onClick={() => setDegradedNotice(null)} className="font-bold underline text-amber-900 hover:text-amber-950">
                    Dismiss
                  </button>
                </div>
              )}
      ...
      {selectedPendingApproval && activeAgent && (
        <DiffModal
          ...
          onRequestRevision={() => handleRequestRevision(selectedPendingApproval.authorId)}
